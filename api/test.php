...
if(!($access_token)){
	...
	$sql="select users.id as uid,users.school_id,users.name,users.position,users.room,users.country_code,users.cellphone,users.landline,users.user_type,users.email from users where users.access_token=:token";
	...
	$sql="select * from school where id=:school_id";
	$sth=$conn->prepare($sql);
	$sth->bindValue('school_id',$school_id);
	try{ $sth->execute();}
	catch(Exception $e){}
	$pol_det=$sth->fetchAll();
	$pol_no=$pol_det[0]['police_contact'];
	
	try{
	DataClass::sendcall($pol_no,$school_id);
	}
	catch(Exception $e){}	
	...
	elseif($status==4){
	 ...
	 /*$sql="update `lockdown` set status=4 where  Date_format(created_on,'%Y-%m-%d')= Date_format(NOW(),'%Y-%m-%d') and lid<=:lid and school_id=:sid";
	  $sth=$conn->prepare($sql);
